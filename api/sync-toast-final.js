...
async function syncBusinessDate(token, date) {
  ...
  try {
    // Step 1: Delete ALL existing data for this date to ensure clean slate
    console.log('  1️⃣ Cleaning ALL existing data for this date...');

    // First, get all order GUIDs for this business date
    ...
    if (existingOrders && existingOrders.length > 0) {
      ...
      // Delete all checks associated with these orders
      const { error: deleteCheckByOrderError } = await supabase
        .from('toast_checks')
        .delete()
        .in('order_guid', orderGuids);

      if (deleteCheckByOrderError) {
        console.error('  ❌ Error deleting checks by order:', deleteCheckByOrderError.message);
      } else {
        console.log(`  ✅ Deleted checks for ${orderGuids.length} orders`);
      }
    }
    ...
    if (deleteOrderError) {
      ...
      console.log(`  ✅ Deleted orders for business date ${businessDate}`);
    }

    // Also delete any checks by date range (in case some don't have order GUIDs)
    ...
    if (deleteCheckError) {
      console.error('  ❌ Error deleting checks by date:', deleteCheckError.message);
    }
        ...
        if (order.checks && Array.isArray(order.checks)) {
          for (const check of order.checks) {
            ...
            if (checkError) {
              ...
              console.error(`     Check data:`, JSON.stringify(checkData, null, 2));
              continue;
            }
