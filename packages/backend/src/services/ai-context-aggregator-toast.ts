...
export class AIContextAggregatorToast {
   ...
   */
  private async getToastAnalytics(
    startDate: Date,
    endDate: Date,
    queryType: string,
  ): Promise<ToastAnalytics> {
    ...
    try {
      ...
      const { data: orders, error: ordersError } = await query.order('business_date', {
        ascending: true,
      });
        ...
        if (overrides && overrides.length > 0) {
          ...
          analytics.dailyBreakdown = overrides.map((override) => {
            ...
            return {
              ...
              avgCheckSize:
                override.check_count > 0 ? override.actual_revenue / override.check_count : 0,
              topItems: [],
              hasOverride: true,
            };
          });
          ...
          analytics.averageCheckSize =
            analytics.totalChecks && analytics.totalRevenue
              ? analytics.totalRevenue / analytics.totalChecks
              : 0;
      ...
      if (overrideMap.size > 0) {
        ...
        analytics.totalRevenue = Array.from(overrideMap.values()).reduce(
          (sum, data) => sum + data.revenue,
          0,
        );
        analytics.totalChecks = Array.from(overrideMap.values()).reduce(
          (sum, data) => sum + data.checkCount,
          0,
        );
        analytics.totalOrders = 0;
        analytics.averageCheckSize =
          analytics.totalChecks > 0 ? analytics.totalRevenue / analytics.totalChecks : 0;
      ...
      if (queryType === 'revenue' && dailyBreakdown.length > 0) {
        ...
        if (lastYearOverrides && lastYearOverrides.length > 0) {
          const lastYearRevenue = lastYearOverrides.reduce(
            (sum, d) => sum + (d.actual_revenue || 0),
            0,
          );
          const lastYearOrders = lastYearOverrides.reduce(
            (sum, d) => sum + (d.check_count || 0),
            0,
          );
            ...
            growth: {
              revenue:
                lastYearRevenue > 0
                  ...
                  : 0,
              orders:
                lastYearOrders > 0 ? ((totalOrders - lastYearOrders) / lastYearOrders) * 100 : 0,
          ...
          if (previousAvg > 0) {
            analytics.yearOverYear.growth.avgCheck =
      ...
      analytics.insights = {
        peakDays: sortedDays
          .slice(0, 3)
          .map((d) => `${d.date} (${d.dayOfWeek}): $${d.revenue.toFixed(2)}`),
        slowDays: sortedDays
          .slice(-3)
          .map((d) => `${d.date} (${d.dayOfWeek}): $${d.revenue.toFixed(2)}`),
        trends: [],
      ...
      if (analytics.dayOfWeekAnalysis) {
        const bestDayOfWeek = Object.entries(analytics.dayOfWeekAnalysis).sort(
          (a, b) => b[1].avgRevenue - a[1].avgRevenue,
        )[0];
        analytics.insights.trends.push(
          `${bestDayOfWeek[0]} is your strongest day averaging $${bestDayOfWeek[1].avgRevenue.toFixed(2)}`,
        );
      }
      ...
      if (analytics.yearOverYear) {
        ...
        if (yoyGrowth > 0) {
          analytics.insights.trends.push(
            `Revenue up ${yoyGrowth.toFixed(1)}% vs same period last year`,
          );
        } else if (yoyGrowth < 0) {
          analytics.insights.trends.push(
            `Revenue down ${Math.abs(yoyGrowth).toFixed(1)}% vs same period last year`,
          );
        }
      }
