...
const kpi_calculator_1 = require("../../src/services/kpi-calculator");
...
async function handler(req, res) {
    ...
    try {
        ...
        const kpiCalculator = new kpi_calculator_1.KPICalculator(supabase);
        ...
        const results = await Promise.allSettled(venues.map(async (venue) => {
            ...
            try {
                ...
                const dailyKPIs = await kpiCalculator.calculateDailyKPIs(venue.id, yesterday);
                // Calculate weekly KPIs (if it's Monday)
                let weeklyKPIs = null;
                if (yesterday.getDay() === 0) { // Sunday
                    const weekStart = new Date(yesterday);
                    weekStart.setDate(weekStart.getDate() - 6);
                    weeklyKPIs = await kpiCalculator.calculateWeeklyKPIs(venue.id, weekStart, yesterday);
                }
                ...
                if (tomorrow.getDate() === 1) {
                    ...
                    monthlyKPIs = await kpiCalculator.calculateMonthlyKPIs(venue.id, monthStart, yesterday);
                }
                // Calculate real-time metrics
                const realtimeMetrics = await kpiCalculator.calculateRealtimeMetrics(venue.id);
...
//# sourceMappingURL=calculate-kpis.js.map