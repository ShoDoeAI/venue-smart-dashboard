      ...
      if (order.checks && Array.isArray(order.checks)) {
        for (const check of order.checks) {
          const checkData = {
            ...
            // Convert dollar amounts to cents for integer columns
            amount: Math.round((check.amount || 0) * 100),
            tax_amount: Math.round((check.taxAmount || 0) * 100),
            tip_amount: Math.round((check.tipAmount || 0) * 100),
            total_amount: Math.round((check.totalAmount || 0) * 100),
            applied_discount_amount: Math.round((check.appliedDiscountAmount || 0) * 100),
            ...
            for (const payment of check.payments) {
              const paymentData = {
                ...
                amount_tendered: payment.amountTendered
                  ? Math.round(payment.amountTendered * 100)
                  : null,
          ...
          // 4. Process selections (items) - table doesn't exist so skip for now
        }
      }
    } catch (error) {
